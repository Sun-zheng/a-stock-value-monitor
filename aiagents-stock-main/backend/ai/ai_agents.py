...
class StockAnalysisAgents:
    ...
    def risk_management_agent(self, stock_info: Dict, indicators: Dict, risk_data: Dict = None) -> Dict[str, Any]:
        ...
        if risk_data and risk_data.get('data_success'):
            # 使用格式化的风险数据
            from backend.data.risk_data_fetcher import RiskDataFetcher
...
"""
    ...
    def run_multi_agent_analysis(self, stock_info: Dict, stock_data: Any, indicators: Dict, 
        ...
        # 创建线程池，最大工作线程数为活跃分析师数量
        max_workers = min(len(active_analysts), 6)  # 最多6个线程
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            if enabled_analysts.get('technical', True):
                future_to_analyst[executor.submit(self.technical_analyst_agent, stock_info, stock_data, indicators)] = "technical"
            
            # 基本面分析
            if enabled_analysts.get('fundamental', True):
                future_to_analyst[executor.submit(self.fundamental_analyst_agent, stock_info, financial_data, quarterly_data)] = "fundamental"
            
            # 资金面分析（传入资金流向数据）
            if enabled_analysts.get('fund_flow', True):
                future_to_analyst[executor.submit(self.fund_flow_analyst_agent, stock_info, indicators, fund_flow_data)] = "fund_flow"
            
            # 风险管理分析（传入风险数据）
            if enabled_analysts.get('risk', True):
                future_to_analyst[executor.submit(self.risk_management_agent, stock_info, indicators, risk_data)] = "risk_management"
            
            # 市场情绪分析（传入市场情绪数据）
            if enabled_analysts.get('sentiment', False):
                future_to_analyst[executor.submit(self.market_sentiment_agent, stock_info, sentiment_data)] = "market_sentiment"
            
            # 新闻分析（传入新闻数据）
            if enabled_analysts.get('news', False):
                future_to_analyst[executor.submit(self.news_analyst_agent, stock_info, news_data)] = "news"
            ...
            for future in concurrent.futures.as_completed(future_to_analyst):
                ...
                try:
                    ...
                    agents_results[analyst] = result
                except concurrent.futures.TimeoutError:
                    print(f"❌ {analyst} 分析超时")
                except Exception as e:
                    print(f"❌ {analyst} 分析出错: {str(e)}")
