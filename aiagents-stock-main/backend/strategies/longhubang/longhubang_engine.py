...
from backend.strategies.longhubang.longhubang_data import LonghubangDataFetcher
from database.managers.longhubang_db import LonghubangDatabase
from backend.ai.longhubang_agents import LonghubangAgents
from backend.strategies.longhubang.longhubang_scoring import LonghubangScoring
...
class LonghubangEngine:
    ...
    def run_comprehensive_analysis(self, date=None, days=1) -> Dict[str, Any]:
        ...
        try:
            ...
            if date:
                # 获取指定日期的数据，启用自动查找最近数据
                data_result = self.data_fetcher.get_longhubang_data(date, auto_find_prev=True)
                
                if data_result:
                    data_list = data_result.get('data', [])
                    # 检查是否使用了不同的日期
                    if 'actual_date' in data_result:
                        actual_date_used = data_result['actual_date']
                        self.logger.info(f"✓ 自动使用最近有数据日期: {actual_date_used}")
                        # 将实际使用的日期保存到结果中
                        results['actual_date_used'] = actual_date_used
                        results['requested_date'] = date
                else:
                    data_list = []
            else:
            ...
            if not data_list:
                ...
                results["error"] = (
                    f"未找到{date if date else '最近'}的龙虎榜数据。\n\n"
                    f"可能原因：\n"
                    f"1. 该日期是非交易日（周末或节假日）\n"
                    f"2. 当日没有股票上榜龙虎榜\n\n"
                    f"💡 建议：尝试选择更早的日期进行分析"
