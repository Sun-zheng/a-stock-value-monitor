...
class LonghubangDataFetcher:
    ...
    def get_longhubang_data(self, date, auto_find_prev=False):
        """
        获取指定日期的龙虎榜数据
        
        Args:
            date: 日期，格式为 YYYY-MM-DD，如 "2023-03-21"
            auto_find_prev: 如果指定日期没有数据，是否自动查找最近的有数据日期
        ...
        if result and result.get('data'):
            ...
            if auto_find_prev:
                print(f"    ⚠️ {date} 无数据，尝试查找最近的有数据日期...")
                # 自动查找最近10天内有数据的日期
                found_result = self._find_recent_data(date, days_back=10)
                if found_result:
                    return found_result
    ...
    def _find_recent_data(self, start_date, days_back=10):
        """
        从指定日期往前查找最近的有数据日期
        
        Args:
            start_date: 开始查找的日期，格式 YYYY-MM-DD
            days_back: 往前查找的天数
        ...
        for i in range(1, days_back + 1):
            ...
            if result and result.get('data'):
                print(f"    ✓ 找到数据！日期: {date_str}, 记录数: {len(result['data'])}")
                # 在结果中添加实际日期信息
                result['actual_date'] = date_str
                result['requested_date'] = start_date
                return result
