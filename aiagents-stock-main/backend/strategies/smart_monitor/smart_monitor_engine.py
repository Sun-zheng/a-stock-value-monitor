...
from backend.strategies.smart_monitor.smart_monitor_deepseek import SmartMonitorDeepSeek
from backend.strategies.smart_monitor.smart_monitor_data import SmartMonitorDataFetcher
from database.managers.smart_monitor_db import SmartMonitorDB
from backend.utils.notification_service import notification_service  # 复用主程序的通知服务
from interface.config.config_manager import config_manager  # 复用主程序的配置管理器
...
class SmartMonitorEngine:
        ...
        try:
            ...
            if self.qmt is not None:
                try:
                    ...
                    self.logger.warning(f"[{stock_code}] 获取账户信息失败（QMT不可用）: {e}")
            ...
            if has_position and position_cost > 0 and position_quantity > 0:
                ...
                try:
                    ...
                    self.logger.warning(f"[{stock_code}] 从QMT获取持仓失败: {e}")
    ...
    def _execute_decision(self, stock_code: str, decision: Dict,
        ...
        try:
            # 如果QMT不可用，返回模拟执行结果
            if self.qmt is None:
                self.logger.info(f"[{stock_code}] QMT未配置，返回模拟执行结果")
                if action in ['BUY', 'SELL', 'HOLD']:
                    return {
                        'success': True,
                        'action': action,
                        'message': f'模拟{action}成功（QMT未配置）',
                        'simulated': True
                    }
                else:
                    return {
    ...
    def _send_notification(self, stock_code: str, stock_name: str,
                          ...
                          position_cost: float = 0, session_info: Dict = None):
        """
        发送通知（使用主程序的通知服务）
...
"""
            ...
            notification_data = {
                ...
                'profit_loss_pct': f"{((market_data.get('current_price', 0) - position_cost) / position_cost * 100):+.2f}" if has_position and position_cost and position_cost > 0 else 'N/A',
                # 交易时段信息
