...
from interface.trading.miniqmt_interface import miniqmt, get_miniqmt_status, QuantStrategyConfig
...
def display_notification_management():
    ...
    # 通知设置
    tab1, tab2, tab3 = st.tabs(["📧 邮件通知", "📱 Webhook通知", "📋 通知历史"])
    ...
    with tab2:
        ...
        if webhook_config['configured']:
            st.success("✅ Webhook配置已完成")
        else:
            st.warning("⚠️ Webhook未配置或配置不完整")
        
        # 显示配置信息
        st.info(f"""
        **当前配置：**
        - Webhook类型: {webhook_config['webhook_type']}
        - Webhook URL: {webhook_config['webhook_url']}
        - 启用状态: {'是' if webhook_config['enabled'] else '否'}
        """)
        ...
        # 测试Webhook按钮
        if webhook_config['configured']:
            if st.button("📱 发送测试Webhook", type="primary", width='stretch'):
                with st.spinner("正在发送测试Webhook..."):
            ...
            st.button("📱 发送测试Webhook", type="primary", width='stretch', disabled=True)
            st.caption("请先在.env文件中配置Webhook参数")
