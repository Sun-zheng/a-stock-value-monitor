...
from backend.strategies.smart_monitor.smart_monitor_engine import SmartMonitorEngine
from database.managers.smart_monitor_db import SmartMonitorDB
from interface.config.config_manager import config_manager  # 使用主程序的配置管理器
...
def render_position_management():
    ...
    # 检查交易接口是否可用
    if qmt is None:
        st.warning("⚠️ 交易接口暂未实现，无法获取账户信息")
        st.info("请在环境配置中启用并配置miniQMT交易接口")
        return
...
def _render_task_kline_and_decisions(task: Dict, db: SmartMonitorDB, engine):
    ...
    Args:
        ...
        engine: 监控引擎实例
    """
    from backend.strategies.smart_monitor.smart_monitor_kline import SmartMonitorKline
    from backend.strategies.smart_monitor.smart_monitor_data import SmartMonitorDataFetcher
