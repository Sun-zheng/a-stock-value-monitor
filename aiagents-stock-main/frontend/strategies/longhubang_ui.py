...
from backend.strategies.longhubang.longhubang_engine import LonghubangEngine
from backend.utils.longhubang_pdf import LonghubangPDFGenerator
...
def display_analysis_results(result):
    ...
    # 检查是否自动使用了不同的日期
    if 'actual_date_used' in result:
        requested_date = result.get('requested_date', '未知')
        actual_date = result.get('actual_date_used', '未知')
        st.warning(
            f"⚠️ 您选择的日期 {requested_date} 没有龙虎榜数据，\n"
            f"系统已自动使用最近有数据的日期：{actual_date}"
        )
