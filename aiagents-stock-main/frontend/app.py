...
from backend.utils.pdf_generator import display_pdf_export_section
from database.managers.database import db
from backend.strategies.monitor.monitor_manager import display_monitor_manager, get_monitor_summary
from backend.strategies.monitor.monitor_service import monitor_service
from backend.utils.notification_service import notification_service
from interface.config.config_manager import config_manager
from frontend.strategies.main_force_ui import display_main_force_selector
from frontend.strategies.sector_strategy_ui import display_sector_strategy
...
FEATURE_CONFIG = {
    ...
    # ==================== 策略分析 ====================
    "sector_strategy": True,    # 智策板块 - AI板块策略分析
    "longhubang": True,       # 智瞰龙虎 - 龙虎榜深度分析
    
    # ==================== 投资管理 ====================
    "portfolio": True,         # 持仓分析 - 投资组合分析与定时跟踪
    "smart_monitor": True,      # AI盯盘 - DeepSeek AI自动盯盘决策交易
    "monitor": True,             # 实时监测 - 价格监控与预警提醒
    
    # ==================== 其他功能 ====================
    "history": True,           # 历史记录 - 查看历史分析记录
...
# 导入登录UI
from frontend.auth.login_ui import LoginUI
...
def main():
    # 检查登录状态
    if not login_ui.check_login_status():
        login_ui.display_login_page()
        return
    ...
    <div class="top-nav">
        ...
        <p class="nav-subtitle">基于LLM的股票投资分析平台</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander("🎯 选股板块", expanded=True):
            ...
            if FEATURE_CONFIG["daily_value"] and st.button("每日价值策略", width='stretch', key="nav_daily_value", help="全市场价值扫描、每日调度、报告交付和历史数据"):
            ...
            # ========== 低价擒牛功能（可通过FEATURE_CONFIG["low_price_bull"]控制） ==========
            if FEATURE_CONFIG["low_price_bull"] and st.button("🐂 低价擒牛", width='stretch', key="nav_low_price_bull", help="低价高成长股票筛选策略"):
                ...
                for key in ['show_history', 'show_monitor', 'show_config', 'show_sector_strategy',
                           ...
                           'show_profit_growth', 'show_index_fund_research', 'show_major_market_etf', 'show_etf_toolkit']:
                    if key in st.session_state:
                        del st.session_state[key]
            
            # ========== 小市值策略功能（可通过FEATURE_CONFIG["small_cap"]控制） ==========
            ...
            # ========== 净利增长功能（可通过FEATURE_CONFIG["profit_growth"]控制） ==========
            if FEATURE_CONFIG["profit_growth"] and st.button("📈 净利增长", width='stretch', key="nav_profit_growth", help="净利润增长稳健股票筛选策略"):
                ...
                for key in ['show_history', 'show_monitor', 'show_config', 'show_sector_strategy',
                           'show_longhubang', 'show_portfolio', 'show_main_force', 'show_low_price_bull',
                           'show_small_cap', 'show_index_fund_research', 'show_major_market_etf', 'show_etf_toolkit']:
                    if key in st.session_state:
                        del st.session_state[key]

        # 📈 ETF板块
        with st.expander("📈 ETF板块", expanded=True):
            st.markdown("**指数基金、ETF筛选、轮动和组合配置工具**")
        ...
        with st.expander("📊 策略分析", expanded=True):
            ...
            # ========== 智策板块功能（可通过FEATURE_CONFIG["sector_strategy"]控制） ==========
            if FEATURE_CONFIG["sector_strategy"] and st.button("🎯 智策板块", width='stretch', key="nav_sector_strategy", help="AI板块策略分析"):
            ...
            # ========== 智瞰龙虎功能（可通过FEATURE_CONFIG["longhubang"]控制） ==========
            if FEATURE_CONFIG["longhubang"] and st.button("🐉 智瞰龙虎", width='stretch', key="nav_longhubang", help="龙虎榜深度分析"):
        ...
        with st.expander("💼 投资管理", expanded=True):
            ...
            # ========== 持仓分析功能（可通过FEATURE_CONFIG["portfolio"]控制） ==========
            if FEATURE_CONFIG["portfolio"] and st.button("📊 持仓分析", width='stretch', key="nav_portfolio", help="投资组合分析与定时跟踪"):
            ...
            # ========== AI盯盘功能（可通过FEATURE_CONFIG["smart_monitor"]控制） ==========
            if FEATURE_CONFIG["smart_monitor"] and st.button("🤖 AI盯盘", width='stretch', key="nav_smart_monitor", help="DeepSeek AI自动盯盘决策交易（支持A股T+1）"):
            ...
            # ========== 实时监测功能（可通过FEATURE_CONFIG["monitor"]控制） ==========
            if FEATURE_CONFIG["monitor"] and st.button("📡 实时监测", width='stretch', key="nav_monitor", help="价格监控与预警提醒"):
        ...
        # ========== 历史记录功能（可通过FEATURE_CONFIG["history"]控制） ==========
        # 📖 历史记录
        if FEATURE_CONFIG["history"] and st.button("📖 历史记录", width='stretch', key="nav_history", help="查看历史分析记录"):
        ...
        # ========== 环境配置功能（可通过FEATURE_CONFIG["config"]控制） ==========
        # ⚙️ 环境配置
        if FEATURE_CONFIG["config"] and st.button("⚙️ 环境配置", width='stretch', key="nav_config", help="系统设置与API配置"):
        ...
        try:
            from database.managers.monitor_db import monitor_db
    ...
    # ========== 历史记录功能检查（可通过FEATURE_CONFIG["history"]控制） ==========
    if FEATURE_CONFIG["history"] and 'show_history' in st.session_state and st.session_state.show_history:
        display_history_records()
        return

    # ========== 实时监测功能检查（可通过FEATURE_CONFIG["monitor"]控制） ==========
    if FEATURE_CONFIG["monitor"] and 'show_monitor' in st.session_state and st.session_state.show_monitor:
        display_monitor_manager()
        return

    # ========== 主力选股功能检查（可通过FEATURE_CONFIG["main_force"]控制） ==========
    if FEATURE_CONFIG["main_force"] and 'show_main_force' in st.session_state and st.session_state.show_main_force:
        display_main_force_selector()
        return
    
    # ========== 低价擒牛功能检查（可通过FEATURE_CONFIG["low_price_bull"]控制） ==========
    if FEATURE_CONFIG["low_price_bull"] and 'show_low_price_bull' in st.session_state and st.session_state.show_low_price_bull:
        from frontend.strategies.low_price_bull_ui import display_low_price_bull
        display_low_price_bull()
        return
    
    # ========== 小市值策略功能检查（可通过FEATURE_CONFIG["small_cap"]控制） ==========
    if FEATURE_CONFIG["small_cap"] and 'show_small_cap' in st.session_state and st.session_state.show_small_cap:
        from frontend.strategies.small_cap_ui import display_small_cap
        display_small_cap()
        return
    
    # ========== 净利增长功能检查（可通过FEATURE_CONFIG["profit_growth"]控制） ==========
    if FEATURE_CONFIG["profit_growth"] and 'show_profit_growth' in st.session_state and st.session_state.show_profit_growth:
        from frontend.strategies.profit_growth_ui import display_profit_growth
        display_profit_growth()
        return

    # ========== 智策板块功能检查（可通过FEATURE_CONFIG["sector_strategy"]控制） ==========
    if FEATURE_CONFIG["sector_strategy"] and 'show_sector_strategy' in st.session_state and st.session_state.show_sector_strategy:
        display_sector_strategy()
        return

    # ========== 智瞰龙虎功能检查（可通过FEATURE_CONFIG["longhubang"]控制） ==========
    if FEATURE_CONFIG["longhubang"] and 'show_longhubang' in st.session_state and st.session_state.show_longhubang:
        display_longhubang()
        return

    # ========== AI盯盘功能检查（可通过FEATURE_CONFIG["smart_monitor"]控制） ==========
    if FEATURE_CONFIG["smart_monitor"] and 'show_smart_monitor' in st.session_state and st.session_state.show_smart_monitor:
        smart_monitor_ui()
        return

    # ========== 持仓分析功能检查（可通过FEATURE_CONFIG["portfolio"]控制） ==========
    if FEATURE_CONFIG["portfolio"] and 'show_portfolio' in st.session_state and st.session_state.show_portfolio:
        from frontend.strategies.portfolio_ui import display_portfolio_manager
        display_portfolio_manager()
        return

    # ========== 环境配置功能检查（可通过FEATURE_CONFIG["config"]控制） ==========
    if FEATURE_CONFIG["config"] and 'show_config' in st.session_state and st.session_state.show_config:
...
def analyze_single_stock_for_batch(symbol, period, enabled_analysts_config=None, selected_model='deepseek-chat'):
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            # 2. 获取财务数据
            fetcher = StockDataFetcher()
            futures['financial'] = executor.submit(fetcher.get_financial_data, symbol)
            ...
            if enable_fundamental and fetcher._is_chinese_stock(symbol):
                def get_quarterly():
                    try:
                        ...
                        return None
                futures['quarterly'] = executor.submit(get_quarterly)
            ...
            if enable_risk and fetcher._is_chinese_stock(symbol):
                def get_risk():
                    try:
                        ...
                        return None
                futures['risk'] = executor.submit(get_risk)
            
            # 收集结果
            for future_name, future in futures.items():
                try:
                    if future_name == 'financial':
                        financial_data = future.result(timeout=30)
                    elif future_name == 'quarterly':
                        quarterly_data = future.result(timeout=30)
                    elif future_name == 'fund_flow':
                        fund_flow_data = future.result(timeout=30)
                    elif future_name == 'sentiment':
                        sentiment_data = future.result(timeout=30)
                    elif future_name == 'news':
                        news_data = future.result(timeout=30)
                    elif future_name == 'risk':
                        risk_data = future.result(timeout=30)
                except concurrent.futures.TimeoutError:
                    print(f"❌ {future_name} 数据获取超时")
                except Exception as e:
                    print(f"❌ {future_name} 数据获取出错: {str(e)}")
...
def run_batch_analysis(stock_list, period, batch_mode="顺序分析"):
    ...
    if batch_mode == "多线程并行":
        ...
        # 使用线程池执行，增加最大并发数以提高分析速度
        # 基于系统资源和API限流考虑，设置为5个并发
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
