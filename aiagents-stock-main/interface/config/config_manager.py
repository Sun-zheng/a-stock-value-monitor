...
class ConfigManager:
    ...
    def __init__(self, env_file: str = ".env"):
        ...
        self.default_config = {
            ...
            "ALIYUN_API_KEY": {
                "value": "",
                "description": "阿里百炼 API密钥",
                "required": False,
                "type": "password"
            },
            "ALIYUN_BASE_URL": {
                "value": "https://dashscope.aliyuncs.com/compatible-mode/v1",
                "description": "阿里百炼 OpenAI兼容地址",
                "required": False,
                "type": "text"
            },
            "SILICONFLOW_API_KEY": {
            ...
            "SILICONFLOW_BASE_URL": {
                ...
                "description": "硅基流动 OpenAI兼容地址",
                "required": False,
                "type": "text"
            },
            "NVIDIA_API_KEY": {
                "value": "",
                "description": "NVIDIA NIM API密钥",
                "required": False,
                "type": "password"
            },
            "NVIDIA_BASE_URL": {
                "value": "https://integrate.api.nvidia.com/v1",
                "description": "NVIDIA NIM OpenAI兼容地址",
                "required": False,
                "type": "text"
            },
            "MODELSCOPE_API_KEY": {
            ...
            "MODELSCOPE_BASE_URL": {
                ...
                "description": "ModelScope OpenAI兼容地址",
                "required": False,
                "type": "text"
            },
            "AI_MODEL_POOL": {
                "value": "",
                "description": "AI模型轮询池（逗号分隔，限流或超时时自动切换）",
                "required": False,
                "type": "text"
            },
            "TUSHARE_TOKEN": {
    ...
    def write_env(self, config: Dict[str, str]) -> bool:
        ...
        try:
            ...
            # 多模型供应商配置
            lines.append("# ========== 多模型供应商配置（OpenAI兼容）==========")
            lines.append(f'ALIYUN_API_KEY="{config.get("ALIYUN_API_KEY", "")}"')
            lines.append(f'ALIYUN_BASE_URL="{config.get("ALIYUN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")}"')
            lines.append(f'SILICONFLOW_API_KEY="{config.get("SILICONFLOW_API_KEY", "")}"')
            lines.append(f'SILICONFLOW_BASE_URL="{config.get("SILICONFLOW_BASE_URL", "https://api.siliconflow.cn/v1")}"')
            lines.append(f'NVIDIA_API_KEY="{config.get("NVIDIA_API_KEY", "")}"')
            lines.append(f'NVIDIA_BASE_URL="{config.get("NVIDIA_BASE_URL", "https://integrate.api.nvidia.com/v1")}"')
            lines.append(f'MODELSCOPE_API_KEY="{config.get("MODELSCOPE_API_KEY", "")}"')
            lines.append(f'MODELSCOPE_BASE_URL="{config.get("MODELSCOPE_BASE_URL", "https://api-inference.modelscope.cn/v1")}"')
            lines.append(f'AI_MODEL_POOL="{config.get("AI_MODEL_POOL", "")}"')
            lines.append("")
